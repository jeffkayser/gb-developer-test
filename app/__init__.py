...
def search(query=None, page_num=1):
    ...
    if page_num < 1:
        page_num = 1
    r = giantbomb.search(query, api_key=app.config['GIANTBOMB_API_KEY'],
                         page_num=page_num)
    results = r.json()
    meta = calc_page(results["offset"], results["number_of_total_results"],
                     results["limit"])
...
def checkout():
    ...
    if request.method == "POST":
        ...
        print("DELETE COOKIE")
        flash(f"Successfully checked out {len(games)}"
              f" game{'' if len(games) == 1 else 's'}. Thank you!")
    else:
        resp = make_response(
            render_template("checkout.html.jinja2",
                            games=games,
